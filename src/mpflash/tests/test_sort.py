...
from mpflash.flash import downloaded_firmwares, find_downloaded_firmware
...
def test_load(tmp_path: Path):
    ...
    all = downloaded_firmwares(test_fw_folder)
    assert len(all) > 0


def test_find():
    test_fw_folder = Path(__file__).parent / "data" / "firmware"
    fws = find_downloaded_firmware(fw_folder=test_fw_folder, port="samd", board="SEEED_WIO_TERMINAL", version="preview")
    assert len(fws) > 0
