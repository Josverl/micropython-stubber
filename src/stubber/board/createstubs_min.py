...
class Stubber:
	...
	def write_object_stub(K,fp,object_expr,obj_name,indent,in_class=0):
		...
		for(E,J,G,Y,a)in X:
			...
			if G=="<class 'type'>"and R(D)<=A2*4:
				...
				H=G[8:-2];C=B
				if H in[s,q,r,t,'bytearray','bytes']:C=W.format(D,E,J,H)
				elif H in[e,d,c]:Z={e:'{}',d:'[]',c:'()'};C=W.format(D,E,Z[H],H)
	...
	def report_start(B,filename='modules.json'):
		H='firmware';B._json_name=y.format(B.path,filename);B._json_first=U;W(B._json_name);A.info('Report file: {}'.format(B._json_name));F.collect()
		try:
			with Q(B._json_name,'w')as G:G.write('{');G.write(dumps({H:B.info})[1:-1]);G.write(f);G.write(dumps({p:{C:__version__},'stubtype':H})[1:-1]);G.write(f);G.write('"modules" :[\n')
		except E as I:A.error(z);B._json_name=D;raise I
	def report_add(B,module_name,stub_file):
		if not B._json_name:raise n(A0)
		try:
...
def _info():
	Y='ev3-pybricks';W='pycopy';U='unix';T='win32';R='arch';Q='cpu';L='ver';F='mpy';E='build';A=h({O:sys.implementation.name,C:B,E:B,L:B,K:sys.platform,S:'UNKNOWN',Q:B,F:B,R:B})
	if A[K].startswith('pyb'):A[K]='stm32'
	elif A[K]==T:A[K]='windows'
	elif A[K]=='linux':A[K]=U
	try:A[C]=A3(sys.implementation.version)
	except I:pass
	...
	for(b,c,d)in[(W,W,'const'),(g,g,'FAT'),(Y,'pybricks.hubs','EV3Brick')]:
