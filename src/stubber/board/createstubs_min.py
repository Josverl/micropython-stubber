...
except U:from ucollections import OrderedDict as d
__version__='v1.12.2'
w=2
x=2
class Stubber:
	...
	def get_obj_attributes(L,item_instance):
		...
		A=k([A for A in A if not A[0].startswith(S)],key=lambda x:x[4]);C.collect();return A,J
...
def _info():
	j='ev3-pybricks';i='pycopy';h='GENERIC';e='arch';c='cpu';a='ver';W='with';I='mpy';G='build';A=d({O:sys.implementation.name,E:B,G:B,a:B,'port':'stm32'if sys.platform.startswith(v)else sys.platform,K:h,c:B,I:B,e:B})
	try:A[E]=L.join([str(A)for A in sys.implementation.version])
	except H:pass
	try:Y=sys.implementation._machine if'_machine'in Q(sys.implementation)else os.uname().machine;A[K]=Y.strip();A[c]=Y.split(W)[1].strip();A[I]=sys.implementation._mpy if'_mpy'in Q(sys.implementation)else sys.implementation.mpy if I in Q(sys.implementation)else B
	except (H,M):pass
	C.collect()
	try:
