...
class Stubber:
	...
	def create_module_stub(J,module_name,file_name=E):
		...
		if H is E:N=D.replace(I,U)+'.py';H=J.path+G+N
		else:N=H.split(G)[-1]
		...
		h(H)
		with S(H,'w')as M:O=Y(J.info).replace('OrderedDict(',C).replace('})','}');Q='"""\nModule: \'{0}\' on {1}\n"""\n# MCU: {2}\n# Stubber: {3}\n'.format(D,J._fwid,O,__version__);M.write(Q);M.write('from typing import Any\nfrom _typeshed import Incomplete\n\n');J.write_object_stub(M,L,D,C)
...
def _info():
	g='ev3-pybricks';e='pycom';d='pycopy';c='unix';b='win32';a='arch';X='ver';K='mpy';G='port';F='build';A=f({N:sys.implementation.name,D:C,F:C,X:C,G:sys.platform,J:'UNKNOWN',V:C,K:C,a:C})
	if A[G].startswith('pyb'):A[G]='stm32'
	elif A[G]==b:A[G]='windows'
	elif A[G]=='linux':A[G]=c
	try:A[D]=I.join([Y(A)for A in sys.implementation.version]).rstrip(I)
	except H:pass
	try:S=sys.implementation._machine if'_machine'in Q(sys.implementation)else os.uname().machine;A[J]=O.join(S.split(O)[:-1]).strip();A[V]=S.split(O)[-1].strip();A[K]=sys.implementation._mpy if'_mpy'in Q(sys.implementation)else sys.implementation.mpy if K in Q(sys.implementation)else C
	except(H,R):pass
