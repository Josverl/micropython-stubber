		...
		for(G,O,T,U,_)in AD:
			...
			if T=="<class 'type'>"and c(E)<=AC*4:
				...
				if P in('str','int','float','bool','bytearray','bytes'):
					...
					else:H=AA.format(E,G,O,P)
				elif P in(p,o,n):AE={p:'{}',o:'[]',n:'()'};H=AA.format(E,G,AE[P],P)
				elif P in('object','set','frozenset','Pin'):H='{0}{1}: {2} ## = {4}\n'.format(E,G,P,T,O)
				elif P=='generator':
					AF=h if Y>0 else C;b=K;A0=B
					if W._use_inspect:
