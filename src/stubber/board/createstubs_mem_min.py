...
class Stubber:
	def __init__(B,path=D,firmware_id=D):
		...
		B.info=_info();A.info('Port: {}'.format(B.info[K]));A.info('Board: {}'.format(B.info[U]));E.collect()
	...
	def create_one_stub(C,module_name):
		...
		I='{}/{}.pyi'.format(C.path,B.replace(J,G));E.collect();D=H
	...
	def report_start(B,filename='modules.json'):
		H='firmware';B._json_name=v.format(B.path,filename);B._json_first=R;X(B._json_name);A.info('Report file: {}'.format(B._json_name));E.collect()
		try:
...
def _info():
	c='ev3-pybricks';Z='pycom';X='pycopy';V='unix';S='win32';R='arch';P='cpu';O='ver';F='mpy';E='build'
	try:H=sys.implementation[0]
	except a:H=sys.implementation.name
	A=g({Q:H,C:B,E:B,O:B,K:sys.platform,U:'UNKNOWN',P:B,F:B,R:B})
	...
	for(e,f,h)in[(X,X,'const'),(Z,Z,'FAT'),(c,'pybricks.hubs','EV3Brick')]:
		try:i=__import__(f,D,D,h);A[Q]=e;del i;break
		except(N,m):pass
