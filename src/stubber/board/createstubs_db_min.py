...
class Stubber:
	...
	def write_object_stub(W,fp,object_expr,obj_name,indent,in_class=0):
		'Write a module/object stub to an open file. Can be called recursive.';AB=' at ...>';AA='{0}{1}: {3} = {2}\n';A7='{}def {}({}) -> {}:\n';A6='bound_method';A4='{}*args, **kwargs';A3='Incomplete';A2='    """{0}"""\n';A1='\\"\\"\\"';u='self';t='    ';p='Exception';k=object_expr;j=' at ';i=', ';h='self, ';Y=in_class;S=fp;E=indent;K.collect()
		if k in W.problematic:C.warning('SKIPPING problematic module:{}'.format(k));return
		...
		for(G,O,T,U,_)in AC:
			...
			if T=="<class 'type'>"and d(E)<=AF*4:
				w=D;x=G.endswith(p)or G.endswith('Error')or G in['KeyboardInterrupt','StopIteration','SystemExit']
				if x:w=p
				H='\n{}class {}({}):\n'.format(E,G,w)
				if x:H+=E+'    ...\n';S.write(H);continue
				S.write(H)
				if W._capture_docstrings:
					try:
						L=U.__doc__
						if L and A5(L,c):
							L=L.strip().replace('"""',A1).replace(e,e+E+t)
				...
				if V is J:V=A4.format(y)
				if A6 in T or A6 in O:H='{}@classmethod\n'.format(E)+A7.format(E,G,V.replace(u,'cls',1),l)
				elif g:H='{}async def {}({}) -> {}:\n'.format(E,G,V,l)
				elif m:H='{}async def {}({}) -> AsyncGenerator:\n'.format(E,G,V)
				elif z:H='{}def {}({}) -> Generator:\n'.format(E,G,V)
				else:H=A7.format(E,G,V,l)
				if W._capture_docstrings:
					try:
						L=U.__doc__
						if L and A5(L,c):
				...
				elif P in(s,r,q):AD={s:'{}',r:'[]',q:'()'};H=AA.format(E,G,AD[P],P)
				elif P in('object','set','frozenset','Pin'):H='{0}{1}: {2} ## = {4}\n'.format(E,G,P,T,O)
				elif P=='generator':
					AE=h if Y>0 else D;b=J;A0=B
					if W._use_inspect:
