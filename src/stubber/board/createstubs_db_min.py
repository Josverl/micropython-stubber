...
class Stubber:
	...
	def create_one_stub(C,module_name):
		...
		H='{}/{}.py'.format(C.path,E.replace(J,G));B.collect();F=D
		try:F=C.create_module_stub(E,H)
		except A:return D
		B.collect();return F
	def create_module_stub(L,module_name,file_name=F):
		H=file_name;E=module_name
		if H is F:O=E.replace(J,U)+'.py';H=L.path+G+O
		else:O=H.split(G)[-1]
		if G in E:E=E.replace(G,J)
		M=F
		try:M=__import__(E,F,F,'*');S=B.mem_free()
		except Q:return D
		h(H)
		with I(H,'w')as N:P=Y(L.info).replace('OrderedDict(',C).replace('})','}');R='"""\nModule: \'{0}\' on {1}\n"""\n# MCU: {2}\n# Stubber: {3}\n'.format(E,L._fwid,P,__version__);N.write(R);N.write('from typing import Any\nfrom _typeshed import Incomplete\n\n');L.write_object_stub(N,M,E,C)
		L._report.append('{{"module": "{}", "file": "{}"}}'.format(E,H.replace('\\',G)))
...
def _info():
	g='ev3-pybricks';e='pycom';d='pycopy';c='unix';b='win32';a='arch';X='ver';I='mpy';G='port';D='build';A=f({O:sys.implementation.name,E:C,D:C,X:C,G:sys.platform,L:'UNKNOWN',V:C,I:C,a:C})
	if A[G].startswith('pyb'):A[G]='stm32'
	elif A[G]==b:A[G]='windows'
	elif A[G]=='linux':A[G]=c
	try:A[E]=J.join([Y(A)for A in sys.implementation.version]).rstrip(J)
	except H:pass
	try:N=sys.implementation._machine if'_machine'in R(sys.implementation)else os.uname().machine;A[L]=P.join(N.split(P)[:-1]).strip();A[V]=N.split(P)[-1].strip();A[I]=sys.implementation._mpy if'_mpy'in R(sys.implementation)else sys.implementation.mpy if I in R(sys.implementation)else C
	except(H,S):pass
	...
	if I in A and A[I]:
		K=int(A[I]);U=[F,'x86','x64','armv6','armv6m','armv7m','armv7em','armv7emsp','armv7emdp','xtensa','xtensawin'][K>>10]
		if U:A[a]=U
		A[I]='v{}.{}'.format(K&255,K>>8&3)
...
def i(info,board_descr,filename):
	with I(filename,'r')as B:
		while 1:
			...
			if C==board_descr:info[L]=E;return K
	return D
def get_root():
	try:B=os.getcwd()
	except(A,H):B=J
...
def main():
	...
	for H in R:
		...
		B.collect();E[H]=Y(stubber._report[-1]if J else L)
		with I(G,'a')as C:C.write('{}={}\n'.format(H,'ok'if J else L))
