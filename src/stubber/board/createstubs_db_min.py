...
class Stubber:
	def __init__(B,path=E,firmware_id=E):
		...
		elif B.info[Q]==v:B._fwid='{family}-v{version}-{port}-{board}'.format(**B.info).rstrip(V)
	...
	def get_obj_attributes(L,item_instance):
		H=item_instance;C=[];K=[]
		for A in N(H):
			if A.startswith(d)and not A in L.modules:continue
			try:
				D=getattr(H,A)
				try:E=b(type(D)).split("'")[1]
				except T:E=B
				if E in{w,x,y,z,e,f,g}:G=1
				elif E in{A0,A1}:G=2
				elif E in'class':G=3
				else:G=4
	...
	def write_object_stub(K,fp,object_expr,obj_name,indent,in_class=0):
		...
		for(E,J,G,Y,a)in X:
			...
			if G=="<class 'type'>"and P(D)<=A5*4:
			...
			elif any(A in G for A in[A1,A0,'closure']):
				S=U;T=B
				if N>0:T='self, '
				...
				H=G[8:-2];C=B
				if H in[y,w,x,z,'bytearray','bytes']:C=W.format(D,E,J,H)
				elif H in[g,f,e]:Z={g:'{}',f:'[]',e:'()'};C=W.format(D,E,Z[H],H)
	...
	def report_start(B,filename=j):
		...
		try:
			with M(B._json_name,h)as G:G.write('{');G.write(dumps({H:B.info})[1:-1]);G.write(k);G.write(dumps({u:{C:__version__},'stubtype':H})[1:-1]);G.write(k);G.write('"modules" :[\n')
		except D as I:A.error(A2);B._json_name=E;raise I
	def report_add(B,module_name,stub_file):
		if not B._json_name:raise s(A3)
		try:
...
def _info():
	Z='ev3-pybricks';X='pycom';V='pycopy';S='unix';R='win32';P='arch';M='cpu';L='ver';F='mpy';D='build';A=l({Q:sys.implementation.name,C:B,D:B,L:B,K:sys.platform,U:'UNKNOWN',M:B,F:B,P:B})
