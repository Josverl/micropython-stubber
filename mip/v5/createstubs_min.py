...
class Stubber:
	...
	def __init__(A,path=D,firmware_id=D):
		...
		A.info=_info();E.info('Port: {}'.format(A.info[I]));E.info('Board: {}'.format(A.info[X]));E.info('Board_ID: {}'.format(A.info[Y]));A._is_low_mem_port=A.info[I]in v;A._capture_docstrings=not A._is_low_mem_port;A._use_inspect=e and not A._is_low_mem_port
	...
	def get_obj_attributes(K,item_instance):
		...
		for A in N(G):
			...
			try:
				C=d(G,A)
				try:E=h(type(C)).split("'")[1]
				except W:E=D
				if E in{'int','float','str','bool',q,r,s}:F=1
				elif E in{A4,A5}:F=2
				elif E in'class':F=3
				else:F=4
				B.append((A,h(C),h(type(C)),C,F))
	...
	def create_module_stub(G,module_name,file_name=J):
		...
		f(H)
		with V(H,'w')as K:
			Q=j(G.info).replace('OrderedDict(',D).replace('})','}');S='"""\nModule: \'{0}\' on {1}\n"""\n# MCU: {2}\n# Stubber: {3}\n'.format(A,G._fwid,Q,__version__);K.write(S);K.write('from __future__ import annotations\nfrom typing import Any, Final, Generator, AsyncGenerator\nfrom _typeshed import Incomplete\n\n')
			if G._is_low_mem_port and A in{A6,'uasyncio.__init__'}:E.warning('Low-memory mode: using shallow stub strategy.');G.write_shallow_stub(K,I)
			else:
	...
	def write_object_stub(W,fp,object_expr,obj_name,indent,in_class=0):
		...
		for(G,P,Y,Q,_)in AK:
			...
			if Y=="<class 'type'>"and k(F)<=AJ*4:
				A3=D;A6=G.endswith(y)or G.endswith('Error')or G in['KeyboardInterrupt','StopIteration','SystemExit']
				if A6:A3=y
				H='\n{}class {}({}):\n'.format(F,G,A3)
				if A6:H+=F+'    ...\n';X.write(H);continue
				X.write(H)
			...
			elif any(A in Y for A in[A5,A4,'closure']):
				u=AC;A7=D
				if b>0:A7=o
				l=B;v=B;A8=B
				if W._use_inspect:
				...
				if W._use_inspect:
					try:
						g=a.signature(Q);A=[];I=B;c=B
						for(O,w)in g.parameters.items():
							T=d(w,A0,J)
						...
						if b>0:Z=o+i.join(A)if A else'self'
						else:Z=i.join(A)
					except S:pass
				...
				if AE in Y or AE in P or hasattr(Q,AF):
					...
					if W._use_inspect:
						try:
							AL=d(Q,AF,Q);g=a.signature(AL);K=[];V=B;x=B
							for(e,AM)in g.parameters.items():
								h=d(AM,A0,J)
								if h==0:V=M;K.append(e)
								elif h==1:
									if V:K.append(C);V=B
									K.append(e)
								elif h==2:
				...
				if R in('str','int','float','bool','bytearray','bytes'):
					...
					if W._use_inspect:
						...
						try:
							...
							if I:A.append(C)
							if b>0 and A and A[0]not in(U,C):A=A[1:]
							if b>0:f=o+i.join(A)if A else'self'
							else:f=i.join(A)
						except S:pass
	...
	def report_start(B,filename='modules.json'):
		...
		try:
			with V(B._json_name,'w')as C:C.write('{');C.write(dumps({D:B.info})[1:-1]);C.write(n);C.write(dumps({A3:{A:__version__},'stubtype':D})[1:-1]);C.write(n);C.write('"modules" :[\n')
		except F as G:E.error(A8);B._json_name=J;raise G
	def report_add(A,module_name,stub_file):
		'Add a module to the report'
		if not A._json_name:raise S(A9)
		try:
...
def AE():
	...
	C=u({Z:B,A:D,P:D,'ver':D,I:sys.platform,X:'UNKNOWN',Y:D,o:D,'cpu':D,Q:D,p:D});return C
def AF(info):
...
def AI(info):
	...
	try:
		...
		elif A in N(sys):B[P]=g(sys.version)
	except(H,W,i):pass
	if B[A]==D and sys.platform not in(t,'win32'):
