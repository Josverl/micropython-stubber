...
except O:from ucollections import OrderedDict as h
__version__='v1.24.0'
A0=2
A1=44
A2=2
...
class Stubber:
	def __init__(B,path=B,firmware_id=B):
		...
		elif B.info[U]==c:B._fwid='{family}-v{version}-{port}-{board_id}'.format(**B.info).rstrip(E)
	...
	def report_start(B,filename='modules.json'):
		G='firmware';B._json_name=w.format(B.path,filename);B._json_first=V;Y(B._json_name);A.info('Report file: {}'.format(B._json_name));F.collect()
		try:
			with R(B._json_name,'w')as E:E.write('{');E.write(dumps({G:B.info})[1:-1]);E.write(g);E.write(dumps({p:{C:__version__},'stubtype':G})[1:-1]);E.write(g);E.write('"modules" :[\n')
		except D as I:A.error(x);B._json_name=H;raise I
...
def _info():
	f='ev3-pybricks';e='pycom';d='pycopy';a='unix';Y='win32';V='arch';S='cpu';R='ver';F='mpy';D='build'
	try:L=sys.implementation[0]
	except b:L=sys.implementation.name
	A=h({U:L,C:B,D:B,R:B,M:sys.platform,T:'UNKNOWN',S:B,F:B,V:B})
	...
	try:
		...
		if G:A[T]=G.split(E)[0];A[z]=G.split(E)[1]if E in G else B
		A[W]=G;A[S]=P.split('with')[-1].strip();A[F]=sys.implementation._mpy if'_mpy'in N(sys.implementation)else sys.implementation.mpy if F in N(sys.implementation)else B
	except(J,Q):pass
	if not A[W]:A5(A)
	try:
		if'uname'in N(os):
	...
	for(i,j,k)in[(d,d,'const'),(e,e,'FAT'),(f,'pybricks.hubs','EV3Brick')]:
		try:l=__import__(j,H,H,k);A[U]=i;del l;break
		except(O,n):pass
	if A[U]==f:A['release']='2.0.0'
	if A[U]==c:
		A[C]
...
def A5(info):
	...
	D[W]=C;D[T]=C.split(E)[0]if E in C else C;D[z]==C.split(E)[1]if E in C else B
def get_root():
	try:A=os.getcwd()
	except(D,J):A=L
	B=A
