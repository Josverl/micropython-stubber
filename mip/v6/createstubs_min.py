...
class Stubber:
	def __init__(B,path=B,firmware_id=B):
		...
		B.info=_info();A.info('Port: {}'.format(B.info[M]));A.info('Board: {}'.format(B.info[Q]));A.info('Board_ID: {}'.format(B.info[V]));F.collect()
		if C:B._fwid=C.lower()
		elif B.info[R]==c:B._fwid='{family}-v{version}-{port}-{board_id}'.format(**B.info).rstrip(E)
	...
	def report_start(B,filename='modules.json'):
		G='firmware';B._json_name=y.format(B.path,filename);B._json_first=W;Y(B._json_name);A.info('Report file: {}'.format(B._json_name));F.collect()
		try:
			with T(B._json_name,'w')as E:E.write('{');E.write(dumps({G:B.info})[1:-1]);E.write(g);E.write(dumps({p:{C:__version__},'stubtype':G})[1:-1]);E.write(g);E.write('"modules" :[\n')
		except D as I:A.error(z);B._json_name=H;raise I
	def report_add(B,module_name,stub_file):
		if not B._json_name:raise o(A0)
		try:
			with T(B._json_name,'a')as C:
	...
	def report_end(B):
		if not B._json_name:raise o(A0)
		with T(B._json_name,'a')as C:C.write('\n]}')
...
def _info():
	e='ev3-pybricks';d='pycom';a='pycopy';Y='win32';W='arch';U='cpu';T='ver';F='mpy';D='build'
	try:L=sys.implementation[0]
	except b:L=sys.implementation.name
	A=i({R:L,C:B,D:B,T:B,M:sys.platform,Q:'UNKNOWN',U:B,F:B,W:B})
	if A[M].startswith('pyb'):A[M]='stm32'
	elif A[M]==Y:A[M]=A1
	elif A[M]=='linux':A[M]=h
	try:A[C]=A6(sys.implementation.version)
	except J:pass
	try:
		S=sys.implementation._machine if'_machine'in N(sys.implementation)else os.uname().machine;A[Q]=S.strip();G=sys.implementation._build if'_build'in N(sys.implementation)else B
		if G:A[Q]=G.split(E)[0];A[A2]=G.split(E)[1]if E in G else B
		A[V]=G;A[U]=S.split('with')[-1].strip();A[F]=sys.implementation._mpy if'_mpy'in N(sys.implementation)else sys.implementation.mpy if F in N(sys.implementation)else B
	except(J,P):pass
	if not A[V]:A7(A)
	try:
		if'uname'in N(os):
	...
	if A[C]==B and sys.platform not in(h,Y):
		try:f=os.uname();A[C]=f.release
		except(P,J,b):pass
	for(g,j,k)in[(a,a,'const'),(d,d,'FAT'),(e,'pybricks.hubs','EV3Brick')]:
		try:l=__import__(j,H,H,k);A[R]=g;del l;break
		except(O,n):pass
	if A[R]==e:A['release']='2.0.0'
	if A[R]==c:
		A[C]
	...
	A[T]=f"{A[C]}-{A[D]}"if A[D]else f"{A[C]}";return A
def A6(version):
...
def A7(info):
	...
	D[V]=C;D[Q]=C.split(E)[0]if E in C else C;D[A2]==C.split(E)[1]if E in C else B
def get_root():
	try:A=os.getcwd()
	except(D,J):A=L
	B=A
...
def j():S("-p, --path   path to store the stubs in, defaults to '.'");sys.exit(1)
def read_path():
	path=B
	if U(sys.argv)==3:
		A=sys.argv[1].lower()
