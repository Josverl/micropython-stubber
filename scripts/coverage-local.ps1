...
# coverage run -m pytest -k mpflash  --junitxml=results/test-results.xml
coverage run -m pytest  --junitxml=results/test-results.xml
...
.\codecov.exe -t ${env:CODECOV_TOKEN} -f results/cover*.lcov
