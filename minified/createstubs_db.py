...
l=2
class Stubber:
	def __init__(self, path=E, firmware_id=E):
		D=firmware_id
		try:
		...
		B.collect()
		if D:
			self._fwid = S(D).lower()
		else:else
			self._fwid = '{family}-{ver}-{port}'.format(**self.info).lower()
		self._start_free = B.mem_free()
		if path:
			if path.endswith(F):path=path[:-1]
		else:path=get_root()
		self.path = f'{path}/stubs/{self.flat_fwid}'.replace('//', F)
		try:Y(path+F)
		except C:
	...
	def get_obj_attributes(self, item_instance):
		...
		for G in dir(F):
			try:
				...
				I.append(f"Couldn't get attribute '{G}' from object '{F}', Err: {K}")
		A=b([B for B in A if not B[0].startswith(W)],key=lambda x:x[4])
		B.collect()
		return A,I
	...
	def create_all_stubs(self):
		...
		for C in self.modules:
			self.create_one_stub(C)
	def create_one_stub(self, module_name):
		A=module_name
		if A in self.problematic:return D
		if A in self.excluded:return D
		H = f'{self.path}/{A.replace(L, F)}.py'
		B.collect()
		J=B.mem_free()
		G('Stub module: {:<25} to file: {:<70} mem:{:>5}'.format(A,H,J))
		I=D
		try:
		...
		B.collect()
		return I
	def create_module_stub(self, module_name, file_name=E):
		J=file_name
		A=module_name
		if A in self.problematic:return D
		if J is E:
			J = self.path + F + A.replace(L,W) + '.py'
		...
		with K(J,'w')as Q:
			...
			self.write_object_stub(Q, P, A, H)
		self._report.append(
			'{{"module": "{}", "file": "{}"}}'.format(A, J.replace('\\', F))
		)
	...
	def write_object_stub(self, fp, object_expr, obj_name, indent, in_class=0):
		...
		D=indent
		B.collect()
		if Q in self.problematic:return
		T,O = self.get_obj_attributes(Q)
		if O:G(O)
		for (E,L,F,W,n) in T:
			if E in['classmethod','staticmethod','BaseException',P]:continue
			if F=="<class 'type'>"and I(D)<=l*4:
				X=H
				Y=E.endswith(P)or E.endswith('Error')or E in['KeyboardInterrupt','StopIteration','SystemExit']
				if Y:X=P
				A = f'\n{D}class {E}({X}):\n'
				if Y:A+=D+'    ...\n';J.write(A);return
				J.write(A)
				self.write_object_stub(J, W, '{0}.{1}'.format(obj_name,E), f'{D}    ', S+1)
				...
				if S>0:a='self, '
				if j in F or j in L:
					A = f'{D}@classmethod\n' + f'{D}def {E}(cls, *args, **kwargs) -> {Z}:\n'
				else:else
					A = f'{D}def {E}({a}*args, **kwargs) -> {Z}:\n'
				A+=D+'    ...\n\n'
				J.write(A)
				...
				if K in[f,d,e,g,'bytearray','bytes']:A=k.format(D,E,L,K)
					...
					if K not in ['object', 'set', 'frozenset']:K=b
					A='{0}{1} : {2} ## {3} = {4}\n'.format(D,E,K,F,L)
				J.write(A)
	...
	def report(self, filename='modules.json'):
		...
		F=',\n'
		G(
			f'Created stubs for {I(self._report)} modules on board {self._fwid}\nPath: {self.path}'
		)
		L = j.format(self.path, filename)
		B.collect()
		try:
			with K(L,'w')as A:
				A.write('{')
				A.write(X({J: self.info})[1:-1])
				A.write(F)
				A.write(X({'stubber':{P:__version__},'stubtype':J})[1:-1])
				A.write(F)
				A.write('"modules" :[\n')
				H=N
				for M in self._report:
					if H:H=D
					else:A.write(F)
					A.write(M)
				A.write('\n]}')
			O = self._start_free - B.mem_free()
...
def _info():
	...
	C='ver'
	B='release'
	X=sys.implementation.name
	Y = 'stm32' if sys.platform.startswith('pyb') else sys.platform
	A={b:X,B:g,P:g,G:H,Q:V,c:V,d:V,F:X,e:Y,f:Y,C:H}
	...
	if sys.platform not in('unix','win32'):
		...
		try:
			...
			if h in D[3]:
				N=D[3].split(h)[0]
				if A[Q]=='esp8266':
					Z = N.split(R)[0] if R in N else N
					A[P]=A[B]=Z.lstrip(K)
				try:A[G]=N.split(R)[1]
				except T:pass
...
def n():
	...
	with K(f'{F}.txt') as A:J=[B.strip()for B in A.read().split('\n')if I(B.strip())and B.strip()[0]!='#']
	B.collect()
	E={}
	try:
		with K(F+H)as A:
	...
	B.collect()
	J=[A for A in J if A not in E.keys()]
	B.collect()
	for L in J:
		P=D
		try:P=stubber.create_one_stub(L)
		except MemoryError:R.reset()
		M = stubber._report[-1] if P else Q
		E[L]=S(M)
		with K(F+H,'a')as A:
			A.write(f'{L}={M}\n')
