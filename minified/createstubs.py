...
class Stubber:
	...
	def create_one_stub(F,module_name):
		D=module_name
		if D.startswith(J)and D!=d:return B
		if D in F.problematic:return B
		if D in F.excluded:return B
		G='{}/{}.py'.format(F.path,D.replace(K,E));C.collect();H=C.mem_free();a('Stub module: {:<20} to file: {:<55} mem:{:>5}'.format(D,G,H))
		try:F.create_module_stub(D,G)
		except A:return B
		C.collect();return N
	def create_module_stub(F,module_name,file_name=D):
		I=file_name;B=module_name
		if B.startswith(J)and B!=d:return
		...
		W(I)
		with G(I,'w')as O:P='"""\nModule: \'{0}\' on {1}\n"""\n# MCU: {2}\n# Stubber: {3}\n'.format(B,F._fwid,F.info,T);O.write(P);O.write('from typing import Any\n\n');F.write_object_stub(O,N,B,H)
	...
	def report(E,filename='modules.json'):
		F=',\n';I=e.format(E.path,filename);C.collect()
		try:
			with G(I,'w')as D:
				D.write('{');D.write(P({'firmware':E.info})[1:-1]);D.write(F);D.write(P({'stubber':{R:T}})[1:-1]);D.write(F);D.write('"modules" :[\n');H=N
				for J in E._report:
					if H:H=B
					else:D.write(F)
					D.write(P(J))
...
def read_path():
	...
	return A
def Y():
	try:A=bytes('abc',encoding=f);C=Y.__module__;return B
	except (Z,F):return N
...
def n():
	...
	for D in C.keys():
		if C[D]!=K:continue
		H=B
		try:H=stubber.create_one_stub(D.decode(f))
		except MemoryError:C.close();E.close();g.reset()
		if H:J='good, I guess'
		else:J=b'skipped'
		C[D]=J;C.flush()
	C.close();E.close()
