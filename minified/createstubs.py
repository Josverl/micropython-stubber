...
class Stubber:
	def __init__(A,path=E,firmware_id=E):
		...
		if E:
			if E.endswith(B):E=E[:-1]
		else:E=get_root()
		A.path='{}/stubs/{}'.format(E,A.flat_fwid).replace('//',B)
		try:R(E+B)
		except D:pass
	...
	def get_obj_attributes(I,item_instance):
		...
		A=[B for B in A if not B[0].startswith(K)];C.collect();return A,G
	...
	def create_one_stub(E,module_name):
		A=module_name
		if A in E.problematic:return I
		if A in E.excluded:return I
		F='{}/{}.py'.format(E.path,A.replace(H,B));C.collect();G=C.mem_free();V('Stub module: {:<20} to file: {:<55} mem:{:>5}'.format(A,F,G))
		try:E.create_module_stub(A,F)
		except D:return I
		C.collect();return P
	def create_module_stub(F,module_name,file_name=E):
		I=file_name;A=module_name
		if A in F.problematic:return
		if I is E:I=F.path+B+A.replace(H,K)+'.py'
		if B in A:A=A.replace(B,H)
		J=E
		try:J=__import__(A,E,E,'*')
		except L:return
		R(I)
	...
	def write_object_stub(K,fp,object_expr,obj_name,indent,in_class=0):
		d='tuple';c='list';b='dict';a='{0}{1} = {2} # type: {3}\n';Z='bound_method';Y='Any';P=in_class;O=object_expr;L='Exception';I=fp;A=indent;C.collect()
		if O in K.problematic:return
		R,g=K.get_obj_attributes(O)
		for (E,J,F,S) in R:
			if E in['classmethod','staticmethod','BaseException',L]:continue
			Q();e(1)
			if F=="<class 'type'>"and N(A)<=f*4:
				T=G;U=E.endswith(L)or E.endswith('Error')
				if U:T=L
				B='\n{}class {}({}):\n'.format(A,E,T);B+=A+"    ''\n"
				if not U:B+=A+'    def __init__(self, *argv, **kwargs) -> None:\n';B+=A+"        ''\n";B+=A+'        ...\n'
				I.write(B);K.write_object_stub(I,S,'{0}.{1}'.format(obj_name,E),A+'    ',P+1)
				...
				H=F[8:-2];B=G
				if H in['str','int','float','bool','bytearray','bytes']:B=a.format(A,E,J,H)
				elif H in[b,c,d]:h={b:'{}',c:'[]',d:'()'};B=a.format(A,E,h[H],H)
				else:
					if not H in['object','set','frozenset']:H=Y
					B='{0}{1} : {2} ## {3} = {4}\n'.format(A,E,H,F,J)
				I.write(B)
			else:I.write("# all other, type = '{0}'\n".format(F));I.write(A+E+' # type: Any\n')
		del R;del g
		try:del E,J,F,S
		except (D,M,W):pass
	@property
	...
	def report(B,filename='modules.json'):
		G='firmware';E=',\n';H=a.format(B.path,filename);C.collect()
		try:
			with open(H,'w')as A:
				A.write('{');A.write(J({G:B.info})[1:-1]);A.write(E);A.write(J({'stubber':{O:__version__},'stubtype':G})[1:-1]);A.write(E);A.write('"modules" :[\n');F=P
				for K in B._report:
					if F:F=I
					else:A.write(E)
					A.write(J(K))
				A.write('\n]}')
			L=B._start_free-C.mem_free()
		except D:pass
def R(path):
	C=path;A=F=0
	while A!=-1:
		...
		F=A+1
def _info():
	h=' on ';g='0.0.0';f='port';e='platform';d='nodename';a='name';U='mpy';T='unknown';R='-';Q='sysname';K='v';J='family';I='build';C='ver';B='release';V=sys.implementation.name;W=sys.platform;A={a:V,B:g,O:g,I:G,Q:T,d:T,S:T,J:V,e:W,f:W,C:G}
	...
	if sys.platform not in('unix','win32'):
		try:
			D=os.uname();A[Q]=D.sysname;A[d]=D.nodename;A[B]=D.release;A[S]=D.machine
			if h in D.version:
				P=D.version.split(h)[0]
				if A[Q]=='esp8266':
	...
	if A[J]==c:
		...
		if A[I]!=G and N(A[I])<4:A[C]+=R+A[I]
	if A[C][0]!=K:A[C]=K+A[C]
	if U in A:
		i=int(A[U]);Z=[E,'x86','x64','armv6','armv6m','armv7m','armv7em','armv7emsp','armv7emdp','xtensa','xtensawin'][i>>10]
		if Z:A['arch']=Z
	return A
def get_root():
	try:A=os.getcwd()
