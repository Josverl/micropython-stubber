...
class Stubber:
	...
	def get_obj_attributes(L,item_instance):
		...
		for G in dir(F):
			try:
				...
				if C in(c,d,e,f,T,U,V):D=1
				elif C in(g,h):D=2
				elif C in'class':D=3
				else:D=4
				A.append((G,Q(B),Q(type(B)),B,D))
			except I as K:J.append("Couldn't get attribute '{}' from object '{}', Err: {}".format(G,F,K))
		A=a([B for B in A if not B[0].startswith(W)],key=lambda x:x[4]);E.collect();return A,J
	...
	def create_module_stub(I,module_name,file_name=C):
		...
		if K is C:K=I.path+D+A.replace(J,W)+'.py'
		...
		I._report.append({'module':A,'file':K})
	...
	def write_object_stub(N,fp,object_expr,obj_name,indent,in_class=0):
		k='{0}{1} = {2} # type: {3}\n';j='bound_method';i='Any';R=in_class;Q=object_expr;P='Exception';I=fp;C=indent;E.collect()
		...
		for (D,M,F,W,n) in S:
			...
			if F=="<class 'type'>"and K(C)<=l*4:
				...
				A='\n{}class {}({}):\n'.format(C,D,X)
				if Y:A+=C+'    ...\n';I.write(A);return
				I.write(A);N.write_object_stub(I,W,'{0}.{1}'.format(obj_name,D),C+'    ',R+1);A=C+'    def __init__(self, *argv, **kwargs) -> None:\n';A+=C+'        ...\n\n';I.write(A)
				...
				elif J in[V,U,T]:m={V:'{}',U:'[]',T:'()'};A=k.format(C,D,m[J],J)
				else:
	...
	def report(C,filename='modules.json'):
		...
		try:
			with R(J,'w')as A:
				A.write('{');A.write(P({I:C.info})[1:-1]);A.write(D);A.write(P({'stubber':{O:__version__},'stubtype':I})[1:-1]);A.write(D);A.write('"modules" :[\n');H=M
				for L in C._report:
					if H:H=F
					else:A.write(D)
					A.write(P(L))
...
def main():
	...
	try:
		with R('modulelist'+'.txt')as C:stubber.modules=[A.strip()for A in C.read().split('\n')if K(A.strip())and A.strip()[0]!='#']
