...
l=2
class Stubber:
	def __init__(self, path=C, firmware_id=C):
		C=firmware_id
		try:
		...
		self.path = f'{path}/stubs/{self.flat_fwid}'.replace('//', D)
		try:X(path+D)
		except B:
	...
	def get_obj_attributes(self, item_instance):
		...
		A=a([B for B in A if not B[0].startswith(V)],key=lambda x:x[4])
		E.collect()
		return A,J
	def add_modules(self, modules):
		self.modules = a(set(self.modules) | set(modules))
	...
	def create_one_stub(self, module_name):
		...
		H = f'{self.path}/{A.replace(J, D)}.py'
		E.collect()
		K=E.mem_free()
		...
		E.collect()
		return I
	def create_module_stub(self, module_name, file_name=C):
		K=file_name
		A=module_name
		if A in self.problematic:return F
		if K is C:
			K = self.path + D + A.replace(J,V) + '.py'
		...
		self._report.append(
			'{{"module": "{}", "file": "{}"}}'.format(A, K.replace('\\', D))
		)
	...
	def write_object_stub(self, fp, object_expr, obj_name, indent, in_class=0):
		...
		for (D,M,F,W,n) in V:
			...
			if F=="<class 'type'>"and K(C)<=l*4:
				...
				if Y:A+=C+'    ...\n';I.write(A);return
				I.write(A)
				self.write_object_stub(I, W, '{0}.{1}'.format(obj_name,D), f'{C}    ', S+1)
				A=C+'    def __init__(self, *argv, **kwargs) -> None:\n'
				A+=C+'        ...\n\n'
				I.write(A)
				...
				if S>0:a='self, '
				if j in F or j in M:
					A = f'{C}@classmethod\n' + f'{C}def {D}(cls, *args, **kwargs) -> {Z}:\n'
				else:else
					A = f'{C}def {D}({a}*args, **kwargs) -> {Z}:\n'
				A+=C+'    ...\n\n'
				I.write(A)
				...
				if J in[e,c,d,f,'bytearray','bytes']:A=k.format(C,D,M,J)
					...
					if J not in ['object', 'set', 'frozenset']:J=i
					A='{0}{1} : {2} ## {3} = {4}\n'.format(C,D,J,F,M)
				I.write(A)
	...
	def report(self, filename='modules.json'):
		...
		D=',\n'
		G(
			f'Created stubs for {K(self._report)} modules on board {self._fwid}\nPath: {self.path}'
		)
		J = i.format(self.path, filename)
		E.collect()
		try:
			with Q(J,'w')as A:
				A.write('{')
				A.write(W({I: self.info})[1:-1])
				A.write(D)
				A.write(W({'stubber':{O:__version__},'stubtype':I})[1:-1])
				A.write(D)
				A.write('"modules" :[\n')
				H=M
				for L in self._report:
					if H:H=F
					else:A.write(D)
					A.write(L)
				A.write('\n]}')
...
def _info():
	...
	D='ver'
	B='release'
	W=sys.implementation.name
	X = 'stm32' if sys.platform.startswith('pyb') else sys.platform
	A={a:W,B:f,O:f,G:H,Q:U,b:U,c:U,F:W,d:X,e:X,D:H}
	...
	if sys.platform not in('unix','win32'):
		...
		try:
			...
			if g in E[3]:
				P=E[3].split(g)[0]
				if A[Q]=='esp8266':
					Y = P.split(R)[0] if R in P else P
					A[O]=A[B]=Y.lstrip(M)
				try:A[G]=P.split(R)[1]
				except S:pass
	...
	if A[F]==j:
		A[D] = A[B][:-2] if A[B]and A[B]>='1.10.0'and A[B].endswith('.0') else A[B]
