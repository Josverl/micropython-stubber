...
class ImphookFileLoader(importlib._bootstrap_external.FileLoader):
    def create_module(self, spec):
        ...
        return _import_hook(spec.name, basename)
...
def setimphook(hook, exts):
    ...
    for i, el in enumerate(sys.path_hooks):
        ...
        warnings.warn(
            "Could not find existing FileFinder to replace, installing ours as the first to use"
        )
        insert_pos = 0

    # Mirrors what's done by importlib._bootstrap_external._install(importlib._bootstrap)
    loaders = [
        (ImphookFileLoader, _import_exts)
    ...
    sys.path_hooks.insert(
        insert_pos, importlib._bootstrap_external.FileFinder.path_hook(*loaders)
    )
